 ...
 * It uses paged memory and chrono, cuda events, and NVTX for timing.
 */

#include "cuda_helper.h"

template<typename T>
void MemCpyPaged<T>::run_chrono( ) {

    for ( int i = 0; i < this->loops; i++ ) {
        this->reset( N, h_a_paged.data( ), h_b_paged.data( ) );

        this->start = std::chrono::high_resolution_clock::now( );

        CUDA_RT_CALL( cudaMemcpy( this->d_a.get( ), h_a_paged.data( ), size, cudaMemcpyHostToDevice ) );
        CUDA_RT_CALL( cudaMemcpy( this->d_b.get( ), h_b_paged.data( ), size, cudaMemcpyHostToDevice ) );
        ...
        CUDA_RT_CALL( cudaMemcpy( h_a_paged.data( ), this->d_a.get( ), size, cudaMemcpyDeviceToHost ) );
        CUDA_RT_CALL( cudaMemcpy( h_b_paged.data( ), this->d_b.get( ), size, cudaMemcpyDeviceToHost ) );

        this->verify( N, h_a_paged.data( ), h_b_paged.data( ) );
...
int main( int argc, char **argv ) {
    ...
    MemCpyPaged<dtype> MemCpyPaged( N );

    double gigabytes { MemCpyPaged.size * 1e-9 };

    printf( "Running with = %lu B (%0.2f GB)\n\n", MemCpyPaged.size, gigabytes );

    // Chrono
    MemCpyPaged.run_chrono( );

    // Events
    MemCpyPaged.run_events( );

    // NVTX
    MemCpyPaged.run_nvtx( );
